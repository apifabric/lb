...
resourcename="lb"  # lower case, only
resourcegroup="lb_rg"
...
# git clone https://github.com/apilogicserver/lb.git
# cd lb
...
az group create --name lb_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group lb_rg --sku S1 --is-linux
az container create --resource-group lb_rg --name lb --image apilogicserver/lb:latest --dns-name-label lb --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//lb.westus.azurecontainer.io:5656'
